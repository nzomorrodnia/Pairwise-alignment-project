...
def get_matrices(n, m):
    ...
    for i in range(1, n+1):
        S[i][0] = GAP_OPEN + (i - 1) * GAP_EXTEND if i == 1 else S[i - 1][0] + GAP_EXTEND
        P[i][0] = GAP_OPEN + (i - 1) * GAP_EXTEND if i == 1 else P[i - 1][0] + GAP_EXTEND
        Q[i][0] = float('-inf')
    for j in range(1, m+1):
        S[0][j] = GAP_OPEN + (j - 1) * GAP_EXTEND if j == 1 else S[0][j - 1] + GAP_EXTEND
        Q[0][j] = GAP_OPEN + (j - 1) * GAP_EXTEND if j == 1 else Q[0][j - 1] + GAP_EXTEND
        P[0][j] = float('-inf')
...
def needleman_wunsch(seq1, seq2):
    ...
    for i in range(1, n+1):
        for j in range(1, m+1):
            ...
            S[i][j] = max(S[i-1][j-1] + match_score, P[i-1][j-1] + match_score, Q[i-1][j-1] + match_score)
